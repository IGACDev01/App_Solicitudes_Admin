...
**App_Solicitudes_Admin** es un sistema de gestión de solicitudes administrativas basado en Streamlit para el Instituto Geográfico Agustín Codazzi (IGAC). Es un dashboard administrativo que gestiona solicitudes a través de diversos flujos de trabajo departamentales, utilizando SharePoint como almacén de datos backend mediante Microsoft Graph API.

**Características principales:**
- Gestión del ciclo de vida de solicitudes a través de 13 procesos departamentales
- Integración con SharePoint mediante Microsoft Graph API
- Notificaciones por correo electrónico para transiciones de estado
- Control de acceso basado en roles por departamento
...
- Sistema automático de "despertar" para prevenir estados inactivos de Streamlit
...
### Ejecutar el Programador de Despertar (Wake-Up Scheduler)

El sistema incluye un programador automático que previene que Streamlit entre en estado inactivo:

```bash
# Windows - hacer doble clic o ejecutar:
...
Ver `Scraper/QUICK_START.md` para configuración como tarea programada de Windows.
...
│   ├── shared_*.py            # Módulos utilitarios (caché, filtros, HTML, zona horaria)
│   └── utils.py               # Utilidades generales
├── Scraper/                   # Sistema programador (sistema separado)
...
- Ejecuta mantenimiento periódico cada 30 minutos (limpieza de caché y estado de sesión)
...
- Métricas de procesos departamentales
...
- `obtener_cache_key()`: Genera clave única para caché
- `invalidar_cache_datos()`: Limpia caché de datos
- `forzar_actualizacion_cache()`: Fuerza actualización de caché
...
- Funciones auxiliares para cálculos temporales
...
- **`proceso`**: Departamento/proceso (ej. "Almacén", "Contabilidad")
...
El sistema gestiona las transiciones de solicitudes a través de flujos de trabajo departamentales:
...
# Credenciales de administrador por departamento
admin_almacen_usuario = "usuario@ejemplo.com"
admin_almacen_password = "contraseña"
# ... más credenciales departamentales
...
La autenticación de SharePoint usa flujo de credenciales de cliente OAuth2. El login de administrador usa email/contraseña específicos por departamento desde secrets.
...
### Agregar un Nuevo Campo a la Visualización de Solicitudes

1. Asegurar que el campo existe en el esquema de lista SharePoint
2. Actualizar `Scripts/admin_solicitudes.py` para mostrar el campo
3. Si el campo necesita conversión de zona horaria, usar funciones de `shared_timezone_utils.py`
4. Si el campo contiene HTML, sanitizar con `shared_html_utils.py`
...
O forzar actualización de clave de caché para función cacheada específica:
...
### El Programador de Despertar No Funciona

1. Verificar que Chrome esté instalado
2. Revisar el archivo de log: `app_wake_up.log`
3. Verificar que la URL de la app sea accesible
4. Asegurar que el botón de despertar existe en la página
5. Ver `Scraper/QUICK_START.md` para configuración detallada
...
- **`Scraper/QUICK_START.md`**: Guía de configuración del programador de despertar
- **`Scraper/SCHEDULER_SUMMARY.md`**: Detalles de implementación del programador
- **`Scraper/WHAT_WAS_DONE.md`**: Contexto histórico sobre configuración del scraper
- **`Scraper/WAKE_UP_SCHEDULER_SETUP.md`**: Guía detallada de configuración como tarea programada
...
2. Verificar logs de la aplicación y SharePoint
3. Consultar documentación adicional en carpeta `Scraper/`
4. Revisar issues conocidos en el sistema de seguimiento del proyecto
